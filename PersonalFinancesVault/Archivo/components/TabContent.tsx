...
const TabContent = ({tab, setNextTabKey, score, setScore}: TabContentProps) => {
  ...
  return (
    <>
        ...
        <Box display='flex' flexDirection='row'>
          {tab.content.options?.map((option: StepOption, index: number) => (
            <Box key={index} padding={1}>
              <Button variant='contained' onClick={() => doButtonAction(option)}>
                {option.label}
              </Button>
            </Box>
          ))}
      ...
      {tab.content.description && (
        <Typography variant='body1'>{tab.content.description}</Typography>
      )}
    </>
  )
}
